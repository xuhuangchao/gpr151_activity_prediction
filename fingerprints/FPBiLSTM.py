...
from keras.layers import Embedding, Conv1D, MaxPooling1D, Dense, Dropout, Flatten, concatenate, LSTM, GRU, Bidirectional
...
if __name__ == "__main__":
    ...
    model.add(Bidirectional(LSTM(128, input_shape=(max_length, 64))))
    ...
    history = model.fit(padded_X_train, y_train_, epochs=50, batch_size=100)
    ...
    source = data_test.dataset.values
    index_02 = []
    index_10 = []
    for i in range(len(source)):
        if source[i] == 2:
            ...
            index_10.append(i)
    y_test_02 = y_test[index_02]
    y_predict_02 = y_predict[index_02]
    # print("AID_1508602数据集的预测结果：")
    # evaluation_class(y_test_02, y_predict_02)

    y_test_10 = y_test[index_10]
    y_predict_10 = y_predict[index_10]
    # print("AID_1508610数据集的预测结果：")
    # evaluation_class(y_test_10, y_predict_10)
