...
from keras.layers import Embedding, Conv1D, MaxPooling1D, Dense, Dropout, Flatten, concatenate, LSTM, CuDNNLSTM
...
if __name__ == "__main__":
    # X_train = np.load("../dataset/mixed_X_train_rdk.npy")
    # X_test = np.load("../dataset/mixed_X_test_rdk.npy")
    ...
    model.add(LSTM(128, input_shape=(max_length, 64)))
    ...
    history = model.fit(padded_X_train, y_train_, epochs=50, batch_size=100, validation_data=(padded_X_test, y_test_))
    ...
    plt.ylim(0.5, 1.0)
    plt.legend()
    plt.savefig("./images/MorganFPLSTM每一轮准确度图片.png")
    plt.show()
    ...
    plt.ylim(0, 1.0)
    plt.legend()
    plt.savefig("./images/MorganFPLSTM每一轮损失值图片.png")
    plt.show()
    ...
    plt.subplot(122)
    plt.scatter(dense_test_tsne[:, 0], dense_test_tsne[:, 1], c=y_test, label="MorganFP+LSTM t-SNE")
    plt.legend()
    ...
    # zinc_similiar.to_csv("ZINC_SIMILIAR_GPR151_v2_MorganFPLSTM_pred.csv", index=False)
